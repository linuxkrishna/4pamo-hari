...
HW_STATUS HW_MMU_AutoIdleEnable(const u32 baseAddress)
{
	...
	status = MMUMMU_SYSCONFIGAutoIdleSet32(baseAddress, HW_SET);
	status = RET_OK;
	return status;
}
	...
	enum HW_SetClear_t       validBit)
{
	...
	CHECK_INPUT_RANGE_MIN0(pageSize, MMU_PAGE_MAX, RET_PARAM_OUT_OF_RANGE,
				RES_MMU_BASE + RES_INVALID_INPUT_PARAM);
	CHECK_INPUT_RANGE_MIN0(mapAttrs->elementSize,
	...
	MMU_SetRAMEntry(baseAddress, physicalAddr,
	mapAttrs->endianism, mapAttrs->elementSize, mapAttrs->mixedSize);
	...
	struct HW_MMUMapAttrs_t    *mapAttrs)
{
	...
	case HW_PAGE_SIZE_4KB:
	pteAddr = HW_MMU_PteAddrL2(pgTblVa, virtualAddr & MMU_SMALL_PAGE_MASK);
	...
	case HW_PAGE_SIZE_64KB:
	numEntries = 16;
	pteAddr = HW_MMU_PteAddrL2(pgTblVa, virtualAddr & MMU_LARGE_PAGE_MASK);
	...
	case HW_PAGE_SIZE_1MB:
	pteAddr = HW_MMU_PteAddrL1(pgTblVa, virtualAddr & MMU_SECTION_ADDR_MASK);
	...
	case HW_PAGE_SIZE_16MB:
	numEntries = 16;
	pteAddr = HW_MMU_PteAddrL1(pgTblVa, virtualAddr & MMU_SSECTION_ADDR_MASK);
	...
	const u32    virtualAddrTag)
{
	...
	mmuCamReg = (mmuCamReg) | (pageSize) |  (validBit << 2) | (preservedBit << 3);
	...
	enum HW_MMUMixedSize_t  mixedSize)
{
	...
	CHECK_INPUT_PARAM(baseAddress, 0, RET_BAD_NULL_PARAM,
					RES_MMU_BASE + RES_INVALID_INPUT_PARAM);
	CHECK_INPUT_RANGE_MIN0(elementSize, MMU_ELEMENTSIZE_MAX, RET_PARAM_OUT_OF_RANGE,
					RES_MMU_BASE + RES_INVALID_INPUT_PARAM);


	mmuRamReg = (physicalAddr & MMU_ADDR_MASK);
	mmuRamReg = (mmuRamReg) | ((endianism << 9) |  (elementSize << 7) | (mixedSize << 6));
