...
#ifdef OMAP44XX
const unsigned long int KMpuMailboxes = 4;
#else
...
irqreturn_t Notify_Mailbox0User0_ISR(int temp, void *anArg, struct pt_regs *p)
{
		...
		volatile unsigned long int pEventStatus = 0;
		...
		for (i = 0; i <  Mailbx_hwConfig.mailboxes[mBoxIndex]; i++) {
				...
				HAL_MBOX_EventStatus(Mailbx_hwConfig.mBoxLinearAddr,
								(enum HAL_MBOX_Id_t)(i), HAL_MBOX_U0_ARM11, (unsigned long *) &pEventStatus);

				if (pEventStatus & HAL_MBOX_INT_ALL) {


						/*Call the respective ISR. The ISR is responsible for reading
						  the mailbox messages and taking action on it */
					if ((Mailbx_swIsrs.Isrs[mBoxIndex][i*HAL_MBOX_ID_WIDTH]) > 0) {

						(*Mailbx_swIsrs.Isrs[mBoxIndex][i*HAL_MBOX_ID_WIDTH])
								(Mailbx_swIsrs.IsrParams[mBoxIndex][i*HAL_MBOX_ID_WIDTH]);


								HAL_MBOX_EventAck(Mailbx_hwConfig.mBoxLinearAddr,
								(enum HAL_MBOX_Id_t)i, HAL_MBOX_U0_ARM11, HAL_MBOX_INT_NEW_MSG);
					}
...
signed long int Mailbx_BindInterrupt(signed long int interruptNo, TIsr hwISR, void *isrArg)
{
		...
		if (status == KErrNone) {
				...
				status =
						request_irq(interruptNo, (void *)Notify_Mailbox0User0_ISR, IRQF_SHARED, "mbox", NotifyMbxDrv_NonShmISR);
...
void Mailbx_Debug(void)
{
		...
		HAL_MBOX_EventStatus(Mailbx_hwConfig.mBoxLinearAddr, (enum HAL_MBOX_Id_t)1, HAL_MBOX_U0_ARM11, &pEventStatus);
...
signed long int Mailbx_DeInit(void)
{
		...
		HAL_OCP_SoftReset(Mailbx_hwConfig.mBoxLinearAddr);
		do {
				HAL_OCP_SoftResetIsDone(Mailbx_hwConfig.mBoxLinearAddr , &temp);
		} while (temp == 0);
...
#if DResource
				if ((i < Mailbx_hwConfig.mBoxModules) && (Mailbx_hwConfig.interruptLines[i] != (-1))) {
...
signed long int Mailbx_Init(void)
{
		...
		Mailbx_hwConfig.interruptLines[(Mailbx_hwConfig.mBoxModules-1)] = KMboxMpuInterrupt;
		Mailbx_hwConfig.mailboxes[(Mailbx_hwConfig.mBoxModules-1)] = KMpuMailboxes;
...
signed long int Mailbx_InterruptDisable(unsigned long int mBoxModuleNo, unsigned long int aIrqBit)
{
		...
		if ((status == KErrNone) && (aIrqBit >= (HAL_MBOX_ID_WIDTH * Mailbx_hwConfig.mailboxes[mBoxModuleNo-1]))) {
				status = KErrArgument;
		}

		/*Interrupts on transmission not supported currently */
		if ((status == KErrNone) && (aIrqBit % HAL_MBOX_ID_WIDTH)) {
				status = KErrNotSupported;
		}

		if (status == KErrNone) {
				HAL_MBOX_EventDisable(Mailbx_hwConfig.mBoxLinearAddr , (enum HAL_MBOX_Id_t)(aIrqBit / HAL_MBOX_ID_WIDTH), HAL_MBOX_U0_ARM11, HAL_MBOX_INT_NEW_MSG);
		}
...
signed long int Mailbx_InterruptEnable(unsigned long int mBoxModuleNo, unsigned long int aIrqBit)
{
		...
		if ((status == KErrNone) && (Mailbx_swIsrs.Isrs[mBoxModuleNo-1][aIrqBit] == NULL)) {
				status = KErrNotReady;
		}
		...
		if (status == KErrNone) {
				HAL_MBOX_EventEnable(Mailbx_hwConfig.mBoxLinearAddr, (enum HAL_MBOX_Id_t)(aIrqBit / HAL_MBOX_ID_WIDTH), HAL_MBOX_U0_ARM11, HAL_MBOX_INT_NEW_MSG);
		}
...
signed long int Mailbx_Read(unsigned long int mBoxModuleNo, unsigned long int aMboxNo, unsigned long int *messages, unsigned long int *numMessages, short int readAll)
{
		...
		if ((status == KErrNone) && (aMboxNo >= Mailbx_hwConfig.mailboxes[mBoxModuleNo-1])) {
				status = KErrArgument;
		}

		if ((status == KErrNone) && (messages == NULL || numMessages == NULL)) {
				status = KErrArgument;
		}

		if (status == KErrNone) {
				/*Read a single message */
				HAL_MBOX_NumMsgGet(Mailbx_hwConfig.mBoxLinearAddr , (enum HAL_MBOX_Id_t)aMboxNo, numMessages);

				if (*numMessages > 0) {
						HAL_MBOX_MsgRead(Mailbx_hwConfig.mBoxLinearAddr, (enum HAL_MBOX_Id_t)aMboxNo, messages);
...
signed long int Mailbx_Register(unsigned long int mBoxModuleNo, unsigned long int aIrqBit, TIsr isrCallbackFn, void *isrCallbackArgs)
{
		...
		TRC_4ENTER("Mailbx_Register", mBoxModuleNo, aIrqBit, isrCallbackFn, isrCallbackArgs);
		...
		if (status == KErrNone) {
				/*Make sure the interrupt is disabled before you plugin a callback */
				HAL_MBOX_EventDisable(Mailbx_hwConfig.mBoxLinearAddr , (enum HAL_MBOX_Id_t)(aIrqBit / HAL_MBOX_ID_WIDTH), HAL_MBOX_U0_ARM11, HAL_MBOX_INT_NEW_MSG);


				/*Store the plugin */
				Mailbx_swIsrs.Isrs[mBoxModuleNo-1][aIrqBit] = isrCallbackFn;
				Mailbx_swIsrs.IsrParams[mBoxModuleNo-1][aIrqBit] = isrCallbackArgs;
...
signed long int Mailbx_Send(unsigned long int mBoxModuleNo, unsigned long int aMboxNo, unsigned long int message)
{
...
#if 0
		if (status == KErrNone) {
				/*Send will loop for 5 ms maximum to transmit a message */

				HAL_MBOX_IsFull(Mailbx_hwConfig.mBoxLinearAddr, (enum HAL_MBOX_Id_t)aMboxNo, (unsigned long *) &mbxFull);

				if (mbxFull) {
						/*TODO: Replace the UTIL_Wait with the Symbian wait code*/
						printk(KERN_ALERT "MBOX full & waiting for 10000 ms\n");
						udelay(1000); /*wait for 1 ms*/
				}
		...
		if (1){
				...
				HAL_MBOX_MsgWrite(Mailbx_hwConfig.mBoxLinearAddr, (enum HAL_MBOX_Id_t)aMboxNo, message);
