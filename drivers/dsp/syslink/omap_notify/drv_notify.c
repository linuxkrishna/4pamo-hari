...
struct semaphore  drv_notify_sem;
 ...
 *----------------------------------------------------------------------------
 */
static struct lst_list *Event_cbckList = NULL ;
 ...
 *----------------------------------------------------------------------------
 */
struct SYNC_CSOBJECT *hEvent_lockHandle;
 ...
 *----------------------------------------------------------------------------
 */
int DRV_ioctl(struct inode *inode, struct file *filp, unsigned int   cmd, unsigned long  args) ;
...
static int __init DRV_initializeModule(void)
{
		...
		if (NOTIFY_SUCCEEDED(status)) {
				...
				Event_cbckList = MEM_Calloc(sizeof(struct lst_list), MEM_NONPAGED);
		...
		if (NOTIFY_SUCCEEDED(status)) {
				for (i = 0 ; i < MAX_PROCESSES ; i++) {
						Event_stateObj [i].bufList = MEM_Calloc(sizeof(struct lst_list), MEM_NONPAGED);
						INIT_LIST_HEAD(&Event_stateObj [i].bufList->head);
...
static void __exit DRV_finalizeModule(void)
{
		...
		for (i = 0 ; i < MAX_PROCESSES ; i++) {
				...
				while (!list_empty ((struct list_head *)Event_stateObj [i].bufList)) {
						packet = (struct Event_Packet_tag *)(Event_stateObj [i].bufList);
				...
				IN OPT void *arg,  IN OPT u32 payload)
{
		...
		if (Event_IsInitialized == TRUE) {
				...
				if (flag != TRUE) {
						...
						uBuf = (struct Event_Packet_tag *)MEM_Alloc(sizeof(struct Event_Packet_tag), MEM_NONPAGED) ;
						if (uBuf != NULL) {
								INIT_LIST_HEAD((struct list_head *)&uBuf->element) ;
								if (DSP_FAILED(status)) {
										...
										spin_lock_irqsave(&drv_notify_lock, irqFlags);
										list_add_tail((struct list_head *)&(uBuf->element), (struct list_head *)Event_stateObj [i].bufList);
...
int DRV_open(struct inode *inode, struct file *filp)
{
		...
		if (Event_IsInitialized == TRUE) {
				...
				spin_unlock_irqrestore (&drv_notify_lock, irqFlags) ;
				if (doInit == 0) {
						...
						if (bufList) {
								...
								if (DSP_SUCCEEDED(status)) {
										spin_lock_irqsave(&drv_notify_lock, irqFlags);
										for (i = 0 ; (i < MAX_PROCESSES) && (flag != TRUE) ; i++) {
												if (Event_stateObj [i].pid == -1) {
														Event_stateObj [i].sem = drv_notify_sem ;
														Event_stateObj [i].pid = pid ;
														Event_stateObj [i].refCount++ ;
														Event_stateObj [i].bufList = bufList ;
														flag = TRUE ;
														break ;
												}
										}
		...
		if (Event_IsInitialized == TRUE) {
				...
				if (flag == TRUE) {
						if (Event_stateObj [i].bufList != NULL) {
								/*Wait for the event */
								if (down_interruptible(&Event_stateObj [i].sem)) {
										retVal = DSP_EFAIL;
										goto func_end;
								}
								spin_lock_irqsave(&drv_notify_lock, irqsave);
								uBuf = (struct Event_Packet_tag *) (Event_stateObj [i].bufList);
								spin_unlock_irqrestore(&drv_notify_lock, irqsave);

								if (uBuf != NULL) {
										retVal = copy_to_user((void *) dst,
														uBuf,
														sizeof(struct Event_Packet_tag)) ;
										...
										if (retVal != 0) {
												...
												retVal = sizeof(struct Event_Packet_tag) ;
										}
										kfree(uBuf) ;
								}
						}
				...
				unsigned long  args)
{
		...
		int                  osStatus = 0       ;
		int                  retVal   = 0       ;
		struct Notify_CmdArgs *srcArgs  = (struct Notify_CmdArgs *) args ;
		...
		if (osStatus == 0) {
				...
				case NOTIFY_DRV_CMD_DRIVERINIT :
								{
										...
										if (retVal == 0) {
												...
												retVal = copy_from_user((void *) config.driverAttrs,
										...
										if (retVal == 0) {
												retVal = copy_from_user(
																(void *) driverName,
																(const void *) uArgs.apiArgs.driverInitArgs.driverName,
																NOTIFY_MAX_NAMELEN) ;
												if (retVal != 0) {
														osStatus = -EFAULT ;
														SET_FAILURE_REASON(NOTIFY_EFAIL) ;
										...
										if (retVal == 0) {
												status = Notify_driverInit(driverName, &config, &handle) ;
												if (NOTIFY_FAILED (status)) {
														SET_FAILURE_REASON(status) ;
												} else {
														uArgs.apiArgs.driverInitArgs.handle = handle ;
												}
										}
								}
								break ;
				...
				case NOTIFY_DRV_CMD_REGISTEREVENT :
								{
										handle = uArgs.apiArgs.registerEventArgs.handle,
											   cbck = MEM_Alloc(sizeof(struct Event_Cbck_tag), MEM_NONPAGED) ;
										if (DSP_FAILED(tmpStatus)) {
												status = NOTIFY_EMEMORY ;
												SET_FAILURE_REASON(status) ;
										} else {
												cbck->procId = uArgs.apiArgs.registerEventArgs.procId ;
												cbck->func  = uArgs.apiArgs.registerEventArgs.fnNotifyCbck ;
												cbck->param = uArgs.apiArgs.registerEventArgs.cbckArg      ;
												...
												if (NOTIFY_FAILED (status)) {
														...
														spin_lock_irqsave(&drv_notify_lock, irqsave);
														INIT_LIST_HEAD(&(cbck->element)) ;
														DBC_Assert(DSP_SUCCEEDED(tmpStatus)) ;
														/*Add the cbck to the event cbck list */
														list_add_tail((struct list_head *)&(cbck->element), (struct list_head *)Event_cbckList);
														spin_unlock_irqrestore(&drv_notify_lock, irqsave);
												}
										}
								}
				...
				case NOTIFY_DRV_CMD_UNREGISTEREVENT :
								{
										handle = uArgs.apiArgs.unregisterEventArgs.handle ;
										tCbck.func  = uArgs.apiArgs.unregisterEventArgs.fnNotifyCbck ;
										tCbck.param = uArgs.apiArgs.unregisterEventArgs.cbckArg      ;
										...
										tmpStatus = omap_list_search(
														...
														(ListMatchFunc) (&Drv_KnlMatchEventCbck)) ;
										spin_unlock_irqrestore(&drv_notify_lock, irqsave);
										if (DSP_FAILED(tmpStatus)) {
												status = NOTIFY_EFAIL ;
												SET_FAILURE_REASON(status) ;
												...
												status = Notify_unregisterEvent(
																handle,
																uArgs.apiArgs.unregisterEventArgs.procId,
																uArgs.apiArgs.unregisterEventArgs.eventNo,
																Notify_drvCallback,
																(void *) cbck) ;
												if (NOTIFY_FAILED (status))
														SET_FAILURE_REASON(status) ;
				...
				case NOTIFY_DRV_CMD_DISABLE :
								{
										uArgs.apiArgs.disableArgs.disableFlags = Notify_disable() ;
								}
								break ;

				case NOTIFY_DRV_CMD_RESTORE :
								{
										status = Notify_restore(uArgs.apiArgs.restoreArgs.restoreFlags) ;
										if (NOTIFY_FAILED (status))
												SET_FAILURE_REASON(status) ;
...
signed long int omap_list_search (IN  struct lst_list *list, IN  void  *data,
				OUT struct list_head **elem, IN  ListMatchFunc  matchFunc)
				...
				if (DSP_SUCCEEDED (status)) {
						...
						while ((found == FALSE) && (temp != NULL)) {
								if ((*matchFunc) (temp, data) == TRUE)
										...
										temp1 = temp ;
										if (temp1->next != (struct list_head *)list)
												temp = temp1->next;
								}
						}
						if (found == TRUE)
