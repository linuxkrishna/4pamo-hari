            ...
            OUT struct DpcObject ** dpcObj)
{
    ...
    if ((userDPCFn == NULL) || (dpcObj == NULL)) {
        ...
        while ((found == FALSE) && (index < MAX_DPC)) {
            if (TEST_BIT (DPC_DpcTaskletMap.usedDPCs, index) == 0u) {
                found = TRUE ;
            }
            else {
                index++ ;
...
DPC_Schedule (IN  struct DpcObject * dpcObj)
{
    ...
    if (IS_OBJECT_VALID (dpcObj, SIGN_DPC) == FALSE) {
        ...
        if (index != INVALID_INDEX) {
            ...
            if (DPC_DpcTaskletMap.dpcs [index].state == TRUE) {
...
DPC_Enable ()
{
    ...
    for (i = 0 ; i < MAX_DPC ; i++) {
        if (IS_OBJECT_VALID (&(DPC_DpcTaskletMap.dpcs [i]), SIGN_DPC)) {
            ...
            if (atomic_read (&DPC_DpcTaskletMap.dpcs [i].pendCount) > 0) {
              tasklet_schedule (&(DPC_DpcTaskletMap.dpcTasklet [i])) ;
            }
        }
    }
}
